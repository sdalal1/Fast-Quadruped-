...
class FullCheetahEnv(gym.Env):
    ...
    def __init__(self, xml_path, render_mode=None):
        ...
        # 11 actuators: 8 for legs + 3 for torso joints
        # self.action_space = spaces.Box(low=-1, high=1, shape=(8,), dtype=np.float32)
        self.action_space = spaces.Box(low=-20, high=20, shape=(8,), dtype=np.float32)
        # self.action_space = spaces.Box(low=-30, high=30, shape=(8,), dtype=np.float32)
        
        
        # Observation space includes joint positions, velocities, and torso orientations
        self.torso_links = ['torso_1', 'torso_2', 'torso_3', 'torso_4']
        obs_dim = self.sim.data.qpos.size + self.sim.data.qvel.size + len(self.torso_links) * 4
